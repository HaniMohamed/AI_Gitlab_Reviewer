...
# Cache lifetime in hours (used by browser localStorage)
CACHE_LIFETIME_HOURS = 24

# Runtime configuration (can be updated via UI) - per-session, not persisted on server
_runtime_config = {
...
def set_gitlab_credentials(url: str, token: str):
    """Set GitLab credentials at runtime (session only, not persisted on server)."""
